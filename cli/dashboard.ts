...
import {
  home, type Run, type Item, type Card, type InboxLine, type Status,
...
export async function dashboard(
    ...
    try {
      ...
      for (;;) {
        ...
        if (key === "e") {
          ...
          if (why) replaceIdea(target.state, target.text, { status: "rejected", text: `${target.text} | ${why}` });
        } else if (key === "d") {
          if (await confirm(`Delete "${cut(target.text, 40)}"?`)) replaceIdea(target.state, target.text, null);
        } else if (key === "r") {
  ...
  async function startBacklogItem(it: Item, from: string) {
    const r = launchRun(it, from);
    if (!r.ok) {
      await report([
        c.bold(r.why.startsWith("No supported") || r.why.startsWith("Terminal.app") ? "Could not open terminal" : "Failed to spawn terminal"),
        "",
        c.dim(r.why),
        "",
        "Run manually:",
    ...
    await report([
      ...
      "",
      c.dim(`Opened in new window running: ${r.agent.cmd} "${r.prompt}"`),
    ]);
  }
...
// How each status is drawn here. The status itself comes from state.ts.
const DOTS: Record<Status | "idea" | "starting", () => string> = {
  abandoned: () => DIM + "○" + OFF,
  waiting: () => c.amber("●"),
  done: () => c.dim("○"),
  running: () => c.cyan("●"),
...
function dotFor(r: Run, now: number): { dot: string; status: string } {
  ...
  return { dot: DOTS[status](), status };
}
type Drawn = Card & { dot: string };
